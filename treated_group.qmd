...
for (yr in 1:10) {
  treated[[paste0("death_", yr, "year")]] <-
    ifelse(!is.na(treated$death_days) & treated$death_days < 365 * yr, 1, 0)
}
```
...
dem <- read.table(
  paste0(rpdr_path, "mes98_093025104659807332_Dem.txt"),
...
post_cre_counts <- cre_labs %>%
  inner_join(treated %>% select(EMPI, last_cre_date), by = "EMPI") %>%
...
## ICI Year Distribution {.unnumbered}

```{r}
#| label: ici-year-histogram
#| include: true
#| fig-cap: "Distribution of ICI start date by year"
treated %>%
  mutate(ici_year = as.numeric(format(Index_date, "%Y"))) %>%
  ggplot(aes(x = ici_year)) +
  geom_histogram(binwidth = 1, fill = "steelblue", color = "white") +
  labs(x = "Year of ICI Start", y = "Count",
       title = "Distribution of ICI Year – Treated Group") +
  theme_minimal()
```
