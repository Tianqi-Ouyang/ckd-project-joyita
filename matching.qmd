...
| Exclude `pre_median_HGB_90days`, `pre_median_ALB_90days`, `pre_median_GHBA1C_90days` from PS model | ~77% missing in controls (routine pre-ICI oncology workup labs are rarely drawn in the control pool); presence would leak treatment status. Shown in Table 1 for transparency only |
...
- **Missing HbA1c**: often the largest source of complete-case drop. If >30% of rows are lost, consider adding a missingness indicator (`ghba1c_missing`) and mean-imputing, rather than dropping.
